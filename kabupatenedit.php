...
<?php
    ...
    if(isset($_POST['Edit'])) {
        ...
        mysqli_query($connection, "UPDATE kabupaten set kabupatennama = '$destinasinama', 
        kabupatenketerangan = '$alamat', provinsiID = '$kodekategori'
            WHERE kabupatenID = '$destinasikode'");
    ...
    $editdestinasi = mysqli_query($connection, "SELECT * FROM kabupaten
    WHERE kabupatenID = '$kodedestinasi'");
    $rowedit = mysqli_fetch_array($editdestinasi);

    $editkategori = mysqli_query($connection, "SELECT * 
        FROM kabupaten, provinsi
        WHERE kabupatenID = '$kodedestinasi' and kabupaten.provinsiID = provinsi.provinsiID");
...
<div class="col-sm-10">
  <input type="text" class="form-control" id="kodedestinasi" name="inputkode" value="<?php echo $rowedit["kabupatenID"]?>" readonly>
</div>
</div>
...
<div class="col-sm-10">
  <input type="text" class="form-control" name="inputnama" id="namadestinasi" value="<?php echo $rowedit["kabupatennama"]?>">
</div>
</div>
...
<div class="col-sm-10">
  <input type="text" class="form-control" name="alamat" id="alamat" value="<?php echo $rowedit["kabupatenketerangan"]?>">
</div>
</div>

<div class="form-group row">
<label for="refkategori" class="col-sm-2 col-form-label">Provinsi Wisata</label>
<div class="col-sm-10">
...
<option value="<?php echo $rowedit["provinsiID"]?>"><?php echo $rowedit['provinsiID']?>
  <?php echo $rowedit2['provinsiNama']?></option>
        ...
        <form method="POST">
            ...
            <table class="table table-hover table-danger">
                ...
                <tbody>
                    ...
                    {
                        $search = $_POST['search'];
                        $query = mysqli_query($connection, "select kabupaten.*, provinsi.provinsiID, provinsi.provinsiNama
                        from kabupaten, provinsi where kabupatennama like '%'.$search'%' and kabupaten.provinsiID = provinsi.provinsiID");
                    }else
                        {
                            $query = mysqli_query($connection, "select kabupaten.*, provinsi.provinsiID, provinsi.provinsiNama
                            from kabupaten, provinsi where kabupaten.provinsiID = provinsi.provinsiID");
                        }
                        ...
                        while ($row = mysqli_fetch_array($query))
                            { ?>
                                <tr>
                                    ...
                                    <td><?php echo $row['kabupatenketerangan'];?></td>
